---
date: "13 Nov. 2021"
description: "Take a deep dive with me and explore a mental model to make our lives easier in dealing with the useEffect hook."
duration: "20 minutes"
...
Let's explore through some examples. Backreading, you might have thought about the refactoring I did in the ticking component before to accommodate the state. I added a `console.log` inside the cleanup function to illustrate a somewhat tricky thing to digest. I highly suggest you run this on your own!
...
As you can see, it's important to know that the cleanup function contains the lexical scope of its render regardless of whether it's being called in succeeding renders. Again, we should think in renders as every effect is tied to one. (*mental model #1*). Let's break down the first three renders substituting the value of `number` at the current render.
...
We can deduce that the cleanup function on every tick will print `n-1` where n is the current value of the state number at any given render. It is recommended that we follow this pattern as much as possible if we intend to have a cleanup where every effect has one since it can be tricky if the cleanup is conditional which will be described in the next section.

Let's check out a trickier example. This component will tick to infinity with varying cleanup functions behind conditions on the second hook.
...
Doing the shoebox mental model up to 7 renders,

!["Shoebox Diagram for TrickierComponent"](/mental-models-with-use-effect/2.svg?w=90)

It can be daunting I know, but if your code is patterned like the one above, you're probably doing something wrong. It's easier to imagine if every render where the effect will run has a corresponding clean up just like the first ticking example above. If you have to print the number on every even number and "hello" on every number divisible by 3 on cleanup, refactor it this way. 
