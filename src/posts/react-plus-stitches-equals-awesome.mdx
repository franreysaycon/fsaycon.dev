...
For simplicity's sake, let's use create-react-app (CRA) for this one. I won't be changing anything out of the ordinary. I will remove all the unnecessary artifacts like CSS and other setups. After the fact, I should only have the `HelloWorld.jsx` in the components folder and the `index.js` that injects the only defined component.
