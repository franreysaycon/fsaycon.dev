...
My approach with any design framework is heavily using design tokens (a concept in originally in System UI), luckily also, available in Stitches. Tokens enable accessing the values specified in a configuration. Before you see that in action, let's define the bread and butter of stitches, the Stitches configuration file. 
